...
<form action="deleteDesTypeFunction.php" method="POST">
...
<div class="card" style="width:950px; margin-top: -0.6%; margin-left: 23.5%;">
  <div class="card-header" style="height:  40px; font-size: 13px; text-align: left; font-weight: 600; color: #747272; background-color: white;">
    List of User Tags
      <a href="addDesignationType.php" class="" style="float: left; margin-top: -20px; width: 15%; height: 33px;margin-left: 90%; font-size: 14px; color: #747272;"><i class="fas fa-plus"></i>&nbsp;Add New</a>
  </div>
        ...
        <table id="userTbl" class="responsive-table table-hover table-striped table-sm m-0" width="100%" cellspacing="0">
          ...
          <tbody>
            <?php
              ...
              $sql = "SELECT * FROM tbl_designation ORDER BY des_id DESC";
              ...
              while($row = $query->fetch_assoc()){
                ...
                "<tr style='font-size: 12px'>
                  <td class=''>
                    <input type='checkbox' class='delete_checkbox' name='user_delete_id[]' value=".$row["des_id"]." />
                  </td>
                  
                  <td>".$row['designation']."</td>
                  <td>
                    <a href='#editDes_".$row['des_id']."' class='btn btn-success btn-sm' data-toggle='modal' style='font-size: 11px'>Edit</a>
                    <a href='#deleteDes_".$row['des_id']."' class='btn btn-danger btn-sm' data-toggle='modal' style='font-size: 11px'>Delete</a>
                  </td>
                </tr>";
