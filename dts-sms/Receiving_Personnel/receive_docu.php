...
$releaser_daw = mysqli_query($conn, "SELECT * from register where department = '$department' and usertype = 'Releasing Officer' ");
        if($releaser_row = mysqli_fetch_array($releaser_daw)) {
          $_SESSION['releaser_id_daw'] = $releaser_row['id'];
...
<head>
  ...
  <title>Receiving Officer | Document Tracking System </title>
  <meta charset="utf-8">
...
<div class="card shadow" style="width: 73%; margin-left: 25%; border: none; margin-top: 1%;">
  ...
  <div class="row">
    ...
    <input type="hidden" class="form-control" id="releaser_name" name="releaser_name">
    <input type="hidden" class="form-control" id="releaser_id" name="releaser_id" value="">
    <input type="hidden" class="form-control" id="receiver_name" name="receiver_name" value="<?php echo $fullname; ?> ">
    ...
    <table id="UnreadTbl" class="responsive-table table-hover table-striped table-sm" width="100%">
          <thead style=" color: #2B2B2B; font-size: 12px; border-bottom: 1.5px solid #A8A8A8;">
            ...
            <tr class="myHead" style="height: 35px">
              <th style="width: 20%; text-align: center; font-size: 11px;">TRACKING NO.</th>
              <th style="width: 20%; text-align: center; font-size: 11px;">FILE NAME</th>
              <th style="width: 18%; text-align: center; font-size: 11px;">MESSAGE/DESCRIPTION</th>
              <th style="width: 20%; text-align: center; font-size: 11px;">SENDER</th>
              <th style="width: 20%; text-align: center; font-size: 11px;">DATE & TIME RECEIVED</th>
          ...
          <tbody>
            <?php
              include_once('connection_db/connection.php');
              $sql = "SELECT receive_file.status as rs, receive_file.releaser_name as rn, receive_file.releaser_id as rl, file_upload.* FROM receive_file INNER JOIN file_upload WHERE receive_file.tracking_no = file_upload.tracking_no AND receive_file.receiver_id = '$id' AND receive_file.status = 'Unread' Order by id DESC";
              //use for MySQLi-OOP
              ...
              while($row = $query->fetch_assoc()){
                ...
                "<tr style='font-size: 12px'>
                  ...
                  <td style='text-align: center; display: none;' id='release_id_".$row['tracking_no']."'>".$row['rl']."</td>
        ...
        <table id="readTbl" class="responsive-table table-hover table-striped table-sm" width="100%" style="display: none;">
          <thead style=" color: #2B2B2B; font-size: 12px; border-bottom: 1.5px solid #A8A8A8;">
            ...
            <tr class="myHead" style="height: 35px">
              <th style="width: 16%; text-align: center; font-size: 11px;">TRACKING NO.</th>
              <th style="width: 20%; text-align: center; font-size: 11px;">FILE NAME</th>
              <th style="width: 18%; text-align: center; font-size: 11px;">MESSAGE/DESCRIPTION</th>
              <th style="width: 20%; text-align: center; font-size: 11px;">SENDER</th>
              <th style="width: 20%; text-align: center; font-size: 11px;">DATE & TIME RECEIVED</th>
              <th style="width: 10%; text-align: center; font-size: 11px;">STATUS</th>
              <th style="width: 10%; text-align: center; font-size: 11px;">MORE</th>
            </tr>
          </thead>
          <tbody>
            <?php
              include_once('connection_db/connection.php');
              $sql = "SELECT receive_file.status as rs, receive_file.releaser_name as rn, file_upload.* FROM receive_file INNER JOIN file_upload WHERE receive_file.tracking_no = file_upload.tracking_no AND receive_file.receiver_id = '$id' AND receive_file.status = 'Read' Order by id DESC";
              //use for MySQLi-OOP
              $query = $conn->query($sql);
              while($row = $query->fetch_assoc()){
                $status = $row['rs'] == 'Unread' ? 'Receive' : 'File Received';
...
function getData(track) {
  ...
  $("#releaser_name").val($("#"+track).text());
  $("input[name=releaser_id]").val($("#release_id_"+track).text());
}
